...
var GATE_NAMESPACE = "tool-lazy-gate";
...
var MAX_LINEAGE_DEPTH = 16;
function parentSessionId(session) {
  const header = session.header;
  const parent = header?.parentSession;
  return typeof parent === "string" && parent.length > 0 ? parent : void 0;
}
function agentsRegistry(ctx) {
  try {
    const agents = ctx.get("agents");
    return agents?.get === void 0 ? void 0 : agents;
  } catch {
    return void 0;
  }
}
function unlockedSkillNamesInState(state) {
  const names = [];
...
function unlockedSkillNamesInLog(session) {
  const names = [];
...
function inheritedUnlockSkillNames(ctx, agent) {
  const registry = agentsRegistry(ctx);
  if (registry === void 0) return [];
  const names = /* @__PURE__ */ new Set();
  const seen = /* @__PURE__ */ new Set();
  let current = agent.session;
  for (let depth = 0; depth < MAX_LINEAGE_DEPTH && current !== void 0; depth += 1) {
    ...
    const parentState = stateBySession.get(parentSession);
    const inherited = parentState?.restored === true ? unlockedSkillNamesInState(parentState) : unlockedSkillNamesInLog(parentSession);
...
function apply(ctx, config = {}) {
  ...
  reflector?.provide(TOOL_LAZY_GATE_SERVICE, {
    ...
    isUnlocked: (agent, skillName) => {
      ...
      const key = capabilityForSkill(state.capabilities, skillName);
      return key === void 0 ? true : state.entries[key]?.unlocked === true;
    }
  });
