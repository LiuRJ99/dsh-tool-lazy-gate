...
function gate(session, agent, capabilities) {
  ...
  for (const [key, cap] of Object.entries(capabilities)) {
    const entry = state.entries[key];
    if (entry === void 0 || entry.unlocked) continue;
...
function apply(ctx, config = {}) {
  ...
  ctx.on("agent/pre-step", ({ agent, messages }, next) => {
    const state = stateBySession.get(agent.session) ?? gate(agent.session, agent, readCapabilities(settingsScope(ctx), config));
  ...
  ctx.on("system-prompt/assemble", async (_assembly, context, next) => {
    ...
    const state = stateBySession.get(agent.session);
    if (state === void 0) return assembled;
    let filtered = false;
